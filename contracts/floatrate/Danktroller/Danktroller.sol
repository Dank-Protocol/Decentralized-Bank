 ...
 */
contract Danktroller is
    DanktrollerV4Storage,
    DanktrollerInterface,
    DanktrollerErrorReporter,
    Exponential
{
    ...
    event NewDankRate(uint256 oldDankRate, uint256 newDankRate);

    /// @notice Emitted when a new DANK speed is calculated for a market
    event DankSpeedUpdated(DToken indexed dToken, uint256 newSpeed);

    /// @notice Emitted when DANK is distributed to a supplier
    event DistributedSupplierDank(
        DToken indexed dToken,
        address indexed supplier,
        uint256 dankDelta,
        uint256 dankSupplyIndex
    );

    /// @notice Emitted when DANK is distributed to a borrower
    event DistributedBorrowerDank(
        DToken indexed dToken,
        address indexed borrower,
        uint256 dankDelta,
        uint256 dankBorrowIndex
    );

    /// @notice Emitted when borrow cap for a dToken is changed
    event NewBorrowCap(DToken indexed dToken, uint256 newBorrowCap);
    ...
    uint256 public constant dankClaimThreshold = 0.001e18;
     ...
     */
    function checkMembership(address account, DToken dToken)
        external
        view
        returns (bool)
    {
     ...
     */
    function enterMarkets(address[] memory dTokens)
        public
        returns (uint256[] memory)
    {
        uint256 len = dTokens.length;

        uint256[] memory results = new uint256[](len);
        for (uint256 i = 0; i < len; i++) {
            DToken dToken = DToken(dTokens[i]);

            results[i] = uint256(addToMarketInternal(dToken, msg.sender));
        }
     ...
     */
    function addToMarketInternal(DToken dToken, address borrower)
        internal
        returns (Error)
    {
     ...
     */
    function exitMarket(address dTokenAddress) external returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) =
            dToken.getAccountSnapshot(msg.sender);
        ...
        uint256 allowed =
            redeemAllowedInternal(dTokenAddress, msg.sender, tokensHeld);
        if (allowed != 0) {
            return
                failOpaque(
                    Error.REJECTION,
    ...
    function redeemAllowed(
        ...
        uint256 allowed = redeemAllowedInternal(dToken, redeemer, redeemTokens);
    ...
    function borrowAllowed(
        ...
        (MathError mathErr, uint256 hypotheticalBorrowAmount) =
            addUInt(DToken(dToken).totalBorrows(), borrowAmount);
        assert(mathErr == MathError.NO_ERROR);

        if (hypotheticalBorrowAmount > borrowCaps[dToken]) {
            return uint256(Error.MARKET_BORROW_LIMIT_REACHED);
        }
    ...
    function liquidateBorrowAllowed(
        ...
        if (
            !markets[dTokenBorrowed].isListed ||
            !markets[dTokenCollateral].isListed
        ) {
            return uint256(Error.MARKET_NOT_LISTED);
        }
        ...
        uint256 borrowBalance =
            DToken(dTokenBorrowed).borrowBalanceStored(borrower);
    ...
    function seizeAllowed(
        ...
        if (
            DToken(dTokenCollateral).danktroller() !=
            DToken(dTokenBorrowed).danktroller()
        ) {
            return uint256(Error.DANKTROLLER_MISMATCH);
        }
    ...
    function transferAllowed(
        ...
        uint256 allowed = redeemAllowedInternal(dToken, src, transferTokens);
        if (allowed != uint256(Error.NO_ERROR)) {
            return allowed;
        }
        ...
        )
    {
        (Error err, uint256 liquidity, uint256 shortfall) =
            getHypotheticalAccountLiquidityInternal(account, DToken(0), 0, 0);
    ...
    function liquidateCalculateSeizeTokens(
        ...
        uint256 priceBorrowedMantissa =
            oracle.getUnderlyingPrice(DToken(dTokenBorrowed));
        uint256 priceCollateralMantissa =
            oracle.getUnderlyingPrice(DToken(dTokenCollateral));
         ...
         */
        uint256 exchangeRateMantissa =
            DToken(dTokenCollateral).exchangeRateStored();
    ...
    function _setCollateralFactor(
        ...
        if (
            ...
            oracle.getUnderlyingPrice(dToken) == 0
        ) {
            return
                fail(
                    Error.PRICE_ERROR,
     ...
     */
    function _supportMarket(DToken dToken) external returns (uint256) {
        if (msg.sender != admin) {
            return
                fail(
                    Error.UNAUTHORIZED,
    ...
    function _setMarketBorrowCaps(
        ...
        uint256 numMarkets = dTokens.length;
    ...
    function _setMintPaused(DToken dToken, bool state) public returns (bool) {
        require(
            markets[address(dToken)].isListed,
        ...
        for (uint256 i = 0; i < allMarkets_.length; i++) {
            ...
            if (markets[address(dToken)].isDanked) {
                Exp memory assetPrice =
                    Exp({mantissa: oracle.getUnderlyingPrice(dToken)});
        ...
        for (uint256 i = 0; i < allMarkets_.length; i++) {
            ...
            uint256 newSpeed =
                totalUtility.mantissa > 0
                    ? mul_(dankRate, div_(utilities[i], totalUtility))
                    : 0;
    ...
    function updateDankSupplyIndex(address dToken) internal {
        ...
        uint256 supplySpeed = dankSpeeds[dToken];
        uint256 blockNumber = getBlockNumber();
        uint256 deltaBlocks = sub_(blockNumber, uint256(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint256 supplyTokens = DToken(dToken).totalSupply();
            uint256 dankAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio =
                supplyTokens > 0
                    ? fraction(dankAccrued, supplyTokens)
                    : Double({mantissa: 0});
            Double memory index =
     ...
     */
    function updateDankBorrowIndex(address dToken, Exp memory marketBorrowIndex)
        internal
    {
        DankMarketState storage borrowState = dankBorrowState[dToken];
        uint256 borrowSpeed = dankSpeeds[dToken];
        uint256 blockNumber = getBlockNumber();
        uint256 deltaBlocks = sub_(blockNumber, uint256(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint256 borrowAmount =
                div_(DToken(dToken).totalBorrows(), marketBorrowIndex);
            uint256 dankAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio =
                borrowAmount > 0
                    ? fraction(dankAccrued, borrowAmount)
                    : Double({mantissa: 0});
            Double memory index =
     ...
     */
    function distributeSupplierDank(
        address dToken,
        address supplier,
        bool distributeAll
    ) internal {
        ...
        Double memory supplierIndex =
            Double({mantissa: dankSupplierIndex[dToken][supplier]});
        ...
        uint256 supplierTokens = DToken(dToken).balanceOf(supplier);
        uint256 supplierDelta = mul_(supplierTokens, deltaIndex);
        uint256 supplierAccrued = add_(dankAccrued[supplier], supplierDelta);
        dankAccrued[supplier] = transferDank(
            supplier,
            supplierAccrued,
            distributeAll ? 0 : dankClaimThreshold
        );
        emit DistributedSupplierDank(
            DToken(dToken),
            supplier,
            supplierDelta,
     ...
     */
    function distributeBorrowerDank(
        address dToken,
        address borrower,
        ...
        Double memory borrowerIndex =
            Double({mantissa: dankBorrowerIndex[dToken][borrower]});
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint256 borrowerAmount =
                div_(
                    DToken(dToken).borrowBalanceStored(borrower),
                    marketBorrowIndex
                );
            uint256 borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint256 borrowerAccrued =
                add_(dankAccrued[borrower], borrowerDelta);
            dankAccrued[borrower] = transferDank(
                borrower,
                borrowerAccrued,
                distributeAll ? 0 : dankClaimThreshold
            );
            emit DistributedBorrowerDank(
                DToken(dToken),
                borrower,
                borrowerDelta,
    ...
    function transferDank(
        ...
        if (userAccrued >= threshold && userAccrued > 0) {
            ...
            uint256 dankRemaining = dank.balanceOf(address(this));
     ...
     */
    function _setDankRate(uint256 dankRate_) public {
        require(adminOrInitializing(), "only admin can change dank rate");

        uint256 oldRate = dankRate;
        dankRate = dankRate_;
    ...
    function _addDankMarketInternal(address dToken) internal {
        ...
        if (
            dankSupplyState[dToken].index == 0 &&
            dankSupplyState[dToken].block == 0
        ) {
            dankSupplyState[dToken] = DankMarketState({
        ...
        if (
            dankBorrowState[dToken].index == 0 &&
            dankBorrowState[dToken].block == 0
        ) {
            dankBorrowState[dToken] = DankMarketState({
                index: dankInitialIndex,
                block: safe32(getBlockNumber(), "block number exceeds 32 bits")
