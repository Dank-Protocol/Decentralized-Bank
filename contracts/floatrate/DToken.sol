...
contract DToken is DTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(
        ...
        // Set the danktroller
        uint256 err = _setDanktroller(danktroller_);
        require(err == uint256(Error.NO_ERROR), "setting danktroller failed");
    ...
    function transferTokens(
        ...
        uint256 allowed =
            danktroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.TRANSFER_DANKTROLLER_REJECTION,
                    allowed
                );
        }
    ...
    {
        uint256 dTokenBalance = accountTokens[account];
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 allowed =
            danktroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.MINT_DANKTROLLER_REJECTION,
                    allowed
                ),
                0
            );
    ...
    function redeemFresh(
        ...
        if (allowed != 0) {
            return
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.REDEEM_DANKTROLLER_REJECTION,
                    allowed
                );
        }
        ...
        returns (uint256)
    {
        ...
        uint256 allowed =
            danktroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.BORROW_DANKTROLLER_REJECTION,
                    allowed
                );
        }
    ...
    function repayBorrowFresh(
        ...
        uint256 allowed =
            danktroller.repayBorrowAllowed(
                address(this),
                payer,
                borrower,
                repayAmount
        ...
        if (allowed != 0) {
            return (
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.REPAY_BORROW_DANKTROLLER_REJECTION,
                    allowed
                ),
                0
            );
    ...
    function liquidateBorrowInternal(
        address borrower,
        uint256 repayAmount,
        DTokenInterface dTokenCollateral
    ...
    function liquidateBorrowFresh(
        ...
        uint256 allowed =
            danktroller.liquidateBorrowAllowed(
                address(this),
        ...
        if (allowed != 0) {
            return (
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.LIQUIDATE_DANKTROLLER_REJECTION,
                    allowed
                ),
                0
            );
        ...
        (uint256 amountSeizeError, uint256 seizeTokens) =
            danktroller.liquidateCalculateSeizeTokens(
                address(this),
                address(dTokenCollateral),
                actualRepayAmount
            );
        ...
        require(
            dTokenCollateral.balanceOf(borrower) >= seizeTokens,
        ...
        if (address(dTokenCollateral) == address(this)) {
            ...
            seizeError = dTokenCollateral.seize(
                liquidator,
                borrower,
                seizeTokens
            );
        ...
        /* We call the defense hook */
        // unused function  // TODO 主网使用了这个函数
        danktroller.liquidateBorrowVerify(
            address(this),
            address(dTokenCollateral),
            liquidator,
            borrower,
    ...
    function seizeInternal(
        ...
        if (allowed != 0) {
            return
                failOpaque(
                    Error.DANKTROLLER_REJECTION,
                    FailureInfo.LIQUIDATE_SEIZE_DANKTROLLER_REJECTION,
                    allowed
                );
        }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setDanktroller(DanktrollerInterface newDanktroller)
        public
        returns (uint256)
    {
        ...
        if (msg.sender != admin) {
            return
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.SET_DANKTROLLER_OWNER_CHECK
                );
        }
